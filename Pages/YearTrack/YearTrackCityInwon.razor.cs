...
{
    public partial class YearTrackCityInwon
    {
        #region Injectors
        [Inject]
        ...
        {
            string url = $"/YearTrackCityInwon_Print?id={memberID}";
            await JSRuntimeInjector.InvokeVoidAsync("open", new object[] { url, "_blank" });

           // await JSRuntimeInjector.InvokeVoidAsync("open", new object[] { $"/YearTrackCityInwon_Print", "_blank" });
        }
