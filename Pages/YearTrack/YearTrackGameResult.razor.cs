...
{
    public partial class YearTrackGameResult
    {
        #region 변수 등 선언
        ...
        List<string?> gNameList = new List<string>() { "육상" };
        ...
        public string? gName { get; set; } = "육상";
        ...
        // 릴레이 관련 변수
        private string? oneSchool { get; set; }
        private string? twoSchool { get; set; }
        private string? threeSchool { get; set; }
        private string? fourSchool { get; set; }
        private string? fiveSchool { get; set; }
        private string? sixSchool { get; set; }
        private string? oneSchoolYear { get; set; }
        private string? twoSchoolYear { get; set; }
        private string? threeSchoolYear { get; set; }
        private string? fourSchoolYear { get; set; }
        private string? fiveSchoolYear { get; set; }
        private string? sixSchoolYear { get; set; }
        ...
        protected async override Task OnInitializedAsync()
        {
            ...
            // 종목
            gNameList = appState.GetGameNames("교육감배 초·중학년별 육상경기대회");

            using var context = _contextFactory.CreateDbContext();
            Z_PartyName? pName = context.Z_PartyNames
                        .Where(it => it.partyName.Contains("교육감배 초·중학년별 육상경기대회")
            ...
            {
                gName = memberName;
                sNameList = appState.GetSectionNames("교육감배 초·중학년별 육상경기대회", gName);
                sName = "종별선택";
                if (pName.etc == "열람" || pName.etc == "마감" || pName.etc == "진행" || pName.etc == "사용")
                {
                    isShowMember = true;
                }
            }
        ...
        // 리스트 종목명 선택시      public void SelectGameName(ChangeEventArgs e)
        ...
        public void SelectSectionName(ChangeEventArgs e)
        {
            ...
            dNameList = appState.GetDetailNames("교육감배 초·중학년별 육상경기대회", gName, sName);
            dName = "세부종목선택";
        ...
        public async Task RankPlayerAdd()
        {
            ...
            else
            {
                ...
                playerList = listPartyEnty.Where(it=>it.city == model.city).Select(it => it.name).ToList();
            }
        }
        ...
        private string NewRecordAddJumsu(string specialOne, string? etc)
        {
            ...
            if (etc.Contains("경북신")) newJumsu = Math.Round(newJumsu * 2.5, 1);
            if (etc.Contains("경북타이")) newJumsu = Math.Round(newJumsu * 2, 1);
            if (etc.Contains("경북부별신")) newJumsu = Math.Round(newJumsu * 1.8, 1);
            if (etc.Contains("경북부별타이")) newJumsu = Math.Round(newJumsu * 1.6, 1);
            if (etc.Contains("대회신")) newJumsu = Math.Round(newJumsu * 1.5, 1);
            if (etc.Contains("대회타이")) newJumsu = Math.Round(newJumsu * 1.2, 1);
        ...
        private void FivePlayer_Changed(ChangeEventArgs e)
        {
            fourPlayer = e.Value.ToString();
            ...
            if (!string.IsNullOrEmpty(model.city) && model.city != "시/군")
            {
                model.name += "," + fivePlayer;
                model.school += "," + entry.schoolName;
                model.schoolYear += "," + entry.schoolYear;
            }
        }

        private void SixPlayer_Changed(ChangeEventArgs e)
        {
            fourPlayer = e.Value.ToString();
        ...
        public void DwonLoadGameResult()
        {
            ...
            {
                string? birth = context.Z_PartyEntries
                                .Where(it => it.partyName == partyName
                                        && it.schoolName == item.school && it.name == item.name
                                        && it.sName == item.sName
                                        && (it.dNameOne == item.dName || it.dNameTwo == item.dName || it.dNameThree == item.dName))
                                .Select(it => it.jumin)
                                .FirstOrDefault();
                if (birth!=null && birth.Length == 6)
                {
                    string result = $"{birth.Substring(0, 2)}.{birth.Substring(2, 2)}.{birth.Substring(4, 2)}.";
                    item.coach = result;
                }
            }
            ...
            using (var package = new ExcelPackage())
            {
                ...
                worksheet.Cells[1, 1].Value = partyName+" 입상자 리스트";
                // 제목 끝
                ...
                FileUtil.SaveAs(JSRuntimeInjector, "학생체전경기결과.xlsx", package.GetAsByteArray());
            }
        }
    }
}
