...
{
    public partial class YearTrackOfficials
    {
        #region 변수 등 선언
        // 인정 가져오기 [1]
        [CascadingParameter]
        ...
        List<string> listGamePosition = new List<string>() {
            "위원장","부위원장","경기임원장","경기부임원장","총무부장","총무부차장","총무부임원","경기부장","경기부차장","경기부임원",
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            {
                // 종목
                gNameList = appState.GetGameNames("초·중학년별 육상경기대회");

                using var context = _contextFactory.CreateDbContext();
                Z_PartyName? pName = context.Z_PartyNames
                            .Where(it => it.partyName.Contains("초·중학년별 육상경기대회")
                                        && (it.etc == "사용" || it.etc == "마감" || it.etc == "열람"))
                            .FirstOrDefault();
                partyName = pName.partyName;
                if (pName.etc == "사용" || memberName == "경북교육청" || memberPart == "경기단체" || memberName == "관리자")
                {
                    isMember = true;
                }
                else
                {
                    isMember = false;
                    await JSRuntimeInjector.InvokeVoidAsync("alert", "경기단체용 입니다. 로그인 후 사용 가능합니다.");
