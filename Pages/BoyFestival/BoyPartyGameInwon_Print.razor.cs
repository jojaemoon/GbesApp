...
{
    public partial class BoyPartyGameInwon_Print
    {
        [Parameter]
        ...
        protected async override Task OnInitializedAsync()
        {
            ...
            if (member.memberName.Contains("학교"))
            {
                listPartyEntry = context.Z_PartyEntries
                                 .Where(it => it.partyName == pName.partyName
                                            && it.schoolName == member.memberName)
                                 .ToList();
            }
            else if (member.memberName.Contains("교육지원청"))
            {
                listPartyEntry = context.Z_PartyEntries
                                 .Where(it => it.partyName == pName.partyName
                                            && it.city == member.city)
            ...
            JSRuntimeInjector.InvokeVoidAsync("topcontainerNoDisplay");  // 주메뉴 없애기
        }
